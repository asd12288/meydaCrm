...
--
-- Storage bucket RLS policies (imports bucket)
--
...
WITH CHECK (
  ...
  AND ((SELECT profiles.role FROM public.profiles WHERE profiles.id = auth.uid()) = 'admin'::public.user_role)
);
...
--
-- Storage bucket RLS policies (exports bucket)
--

CREATE POLICY "Admin can upload export files"
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (
...
CREATE POLICY "Admin can read export files"
ON storage.objects FOR SELECT TO authenticated
USING (
  (bucket_id = 'exports'::text)
  AND ((SELECT profiles.role FROM public.profiles WHERE profiles.id = auth.uid()) = 'admin'::public.user_role)
...
CREATE POLICY "Admin can delete export files"
ON storage.objects FOR DELETE TO authenticated
USING (
  (bucket_id = 'exports'::text)
  AND ((SELECT profiles.role FROM public.profiles WHERE profiles.id = auth.uid()) = 'admin'::public.user_role)
...
--
-- Storage bucket RLS policies (avatars bucket - public read)
--
